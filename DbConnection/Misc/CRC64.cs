...
namespace DbConnection.Misc;

public static class CRC64
{
    static ulong[] CrcTable64 = new ulong[]
    {
    ...
    private static ulong Crc64(string Data, int Index)
    {
        while (true)
        {
            if (Index == -1)
            ...
            {
                ulong Next = Crc64(Data, Index - 1);
                return (Next >> 8) ^ CrcTable64[(Next ^ Data[Index]) & 0xFF];
            }
        }
    }

    public static ulong Generate64(string? Data)
    {
        if (Data == null)
        {
            return 0;
        ...
        {
            Val = (Val >> 8) ^ CrcTable64[(Val ^ Data[N]) & 0xFF];
        }
